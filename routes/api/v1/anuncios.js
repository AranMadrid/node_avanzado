    ...
    async(req, res, next) => {
        try {
            ...
            if (typeof req.file === 'undefined') {
                const err = new Error('The photo should be file object.');
                err.status = 422;
                next(err);
                return;
            }
            ...
            if (!(RegexImageExtension.test(req.file.originalname.toLowerCase()))) {
                const err = new Error(`Not valid (${req.file.originalname}). The allowed extensions for photo are: gif|jpg|jpeg|tiff|png|webp|bmp`);
                err.status = 422;
                next(err);
                return;
            }
